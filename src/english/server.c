...
void kick_client(const char *username) {
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (clients[i].sock != 0 && strcmp(clients[i].username, username) == 0) {
            ...
            snprintf(buffer, sizeof(buffer), "INFO: You have been disconnected from the server.\n");
            send(clients[i].sock, buffer, strlen(buffer), 0);
            printf(RED "User '%s' was kicked.\n" RESET, username);
...
void add_client(int client_sock, const char *username) {
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (clients[i].sock == 0) {
            ...
            snprintf(log, sizeof(log), "User '%s' joined.", username);
            log_message(log);
            break;
        }
    }
...
void remove_client(int client_sock) {
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (clients[i].sock == client_sock) {
            char log[BUFFER_SIZE];
            snprintf(log, sizeof(log), "User '%s' disconnected.", clients[i].username);
            log_message(log);
            clients[i].sock = 0;
            break;
        }
...
void *delayed_shutdown(void *arg) {
    ...
    char buffer[BUFFER_SIZE];
    snprintf(buffer, sizeof(buffer), "INFO: The server will shut down in %d seconds.\n", seconds);
    broadcast_message(buffer);

    sleep(seconds);
    printf(RED "Server is shutting down now...\n" RESET);
    exit(0);
}
        ...
        if (fgets(command, sizeof(command), stdin) != NULL) {
            ...
            if (strcmp(command, "-list") == 0) {
                ...
                if (username) {
                    ...
                    printf(RED "Error: No username provided.\n" RESET);
                }
                ...
                if (message) {
                    ...
                    printf(RED "Error: No message provided.\n" RESET);
                }
                ...
                if (seconds_str) {
                    ...
                    if (seconds > 0) {
                        ...
                        *arg = seconds;
                        ...
                        printf(RED "Error: Invalid time value.\n" RESET);
                    }
                } else {
                    printf(RED "Error: No time specified.\n" RESET);
                }
            } else if (strcmp(command, "-info") == 0) {
                printf("  Server version: " CYAN "%s\n" RESET, SERVER_VERSION);
                printf("Active connections: " CYAN "%d\n" RESET, count_clients());
                printf("      Leth\n\n");
            } else if (strcmp(command, "-help") == 0) {
                printf("Available commands:\n");
                printf(" " CYAN "-list" RESET "                  Show active users\n");
                printf(" " CYAN "-kick <user>" RESET "           Disconnect a user\n");
                printf(" " CYAN "-broadcast <message>" RESET "   Send a message to all users\n");
                printf(" " CYAN "-shutdown w <seconds>" RESET " Execute a delayed shutdown\n");
                printf(" " CYAN "-info" RESET "                  Show server information\n");
                printf(" " CYAN "-help" RESET "                  Show this help\n\n");
            } else {
                printf(RED "Unknown server command: %s\n" RESET, command);
            }
        }
    }
    return NULL;
...
int main() {
    ...
    printf(BLUE "Server running on version %s, port %d.\n" RESET, SERVER_VERSION, PORT);
